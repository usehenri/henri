...
| `agents`                           | `AGENTS.md` (generated from the application, see below), `CLAUDE.md` and `.mcp.json`. See [Coding agents](/guides/agents/).                                                                                                                                |
...
`agents` is the one generator that reads the application instead of taking arguments: `AGENTS.md` is built from the renderer, the stores, the models and their marks, the expanded routes, the controllers, the policies, the jobs, the mailers, the workers and the henri packages that are installed, so a section with nothing true to say is not written at all. Only the region between `<!-- henri:agents ... -->` and `<!-- /henri:agents -->` is ever rewritten; text around it is copied through byte for byte. It reports `updated` for a rewritten region, `created` for a file it had to write, and `skipped` — with the reason — when the region was edited by hand or when the file carries no region at all, neither of which it will touch without `--force`. `henri new` writes the file through the same call, so a scaffolded `AGENTS.md` and a regenerated one are the same bytes. See [Coding agents](/guides/agents/#agentsmd-is-generated).

`skills` reads the application the same way and writes `.claude/skills/<name>/SKILL.md`, one file per procedure that has an order: adding a model, adding a resource end to end, the pre-commit gates, diagnosing a `HENRI_*` failure, and driving the running application through the MCP server. The procedure is generic and ships in the package; the facts are this application's, so a Drizzle store gets `henri db:generate`, an mssql store is told it has no migrations and `henri db:status --sql` is what reports the drift, and `config.tenancy` adds the question of whose rows a new model holds. `--for <vendor>` picks the layout (`claude` is the default and the only one so far). The YAML frontmatter has to open the first line of a `SKILL.md`, so the generated region starts below it and the `description` — the line that decides when a skill loads — is written once and never rewritten; the region itself follows the same rules and the same `--force` as `AGENTS.md`. Nothing asks a model for anything: the input is the directory and the output is bytes. See [Coding agents](/guides/agents/#skills-are-the-order-things-happen-in).

`authentication`, `agents` and `skills` take no name. `authentication` is the one generator that edits `config/*.json`: it adds the three account blocks to `user` in every configuration file that does not have them, and reports the files it changed (`updated` in the `--json` summary). It writes nothing that reimplements a token, a hash or a session — the endpoints are the ones the user module mounts (see [Users](/guides/users/#the-account-flows)) — and its pages take the same renderer fork as the scaffolded ones.
...
They also declare what their two writes accept. `create` and `update` get a [`params`](/guides/controllers/#params-what-an-action-accepts) block — one selector, the fields `FIELDS` permits, typed from the model file — so a request of the wrong shape is a `422` naming the field before the action runs, and what is accepted is [written back where it came from](/guides/controllers/#coercion): `req.permit(...FIELDS)` hands the action `true` rather than the string `"true"`, and `false` rather than the string `"false"`, which every adapter stores as `false` and JavaScript reads as truthy. What is in the block, and what is not, is argued in the file itself:

| The model says                                   | The block says                                                                                                                                                                                                                                                      |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type: 'boolean'`, `'decimal'`, `'date'`, …      | the type, in the short form (`done: 'boolean'`). A JSON body is checked and never parsed, so a client that sends `"true"` where a boolean was declared is refused rather than guessed at                                                                            |
| `enum: ['draft', 'live']`                        | the values, copied. Unlike a page, this block cannot be handed `Model.enums`: a controller is compiled at runlevel 2 and the models are built at 3, so a literal is the only thing that can be written there, and the file says so                                  |
| `required: true`                                 | nothing. A parameter is required when the _key arrived_, and a model asks Rails' presence — the empty string a form posts for an untouched input passes the first and is refused by the second, so the same word would be two rules                                 |
| `references: { model: 'Event' }`, Mongoose `ref` | nothing, and the comment names the column. henri publishes a foreign key as the target's `externalId`, so only the application knows whether a request carries that or the column's own value — the reason `henri openapi` leaves one untyped in a request body too |

Every other action declares nothing: an `index` takes its page from `req.pagination()`, and `:id` is a path parameter the `before` hook already answers a `404` for — the slug of a model that has one included, which is not a uuid. A resource with nothing to type gets no block rather than an empty one, and `henri openapi` reads whatever is there, so `POST /tasks` is described by the declaration instead of by the model's writable columns.
...
henri db:schema [--table=<name>]                                                  [--store=<name>] [--json]
...
Every one of those reads the SQL it is about to write or run and reports what would hurt a database that has rows in it: a dropped or renamed column or table, a `NOT NULL` column with no default, a type change, an index build on postgres, and a `DELETE` or `UPDATE` with no `WHERE`. `generate` warns and writes the file anyway. `migrate` **refuses in production** — and so does a production boot with `"migrate": true` on the store — until the migration's token is in [`migrations.approved`](/configuration/#the-migrations-object); the refusal exits with `1` and `HENRI_MIGRATION_UNREVIEWED`, applies nothing at all, and names every finding with the safer way to do it. `status` prints the same review for each pending migration, with the token to approve it. `--json` adds `findings` and `token` to `generate`, and `review` (a list of `{ tag, findings, token, approved }`) to `status` and `migrate`. See [Migration safety](/guides/models/#migration-safety).
...
`db:schema` prints what the database **holds**, which is the question `db:status` does not answer: a clean `db:status` says "up to date" and nothing else, and a `mongoose` store has never had an answer there at all. It is read from the server, never from `app/models` — the physical table name, the real column names (so the `externalId` a model declares shows up as the `external_id` it is), the type the dialect chose, whether each column is nullable, its default, the values of an enum where the dialect keeps them, every index that exists including the ones nobody declared, and the names of the tables no model claims (henri's own `henri_jobs` and `henri_trail`, the sessions, drizzle's migration journal, or something a person made). Those are named and not described: `DESCRIBE <table>` through [the `query` tool](/guides/agents/#asking-the-running-application) is how to look inside one. Nothing is written, and nothing is created — a table that is not there answers `exists: false` rather than being made on the way past. `--table=<name>` narrows it to one table or one model. On a `mongoose` store the answer says `enforced: false` and `read: "models"`, and a `note` says why: the collections and the indexes are read from the server and are real, the fields are henri's own declaration applied by Mongoose, and MongoDB holds no document to them. `--json` answers `{ store, adapter, kind, dialect, read, enforced, tables, unclaimed }`, where a table is `{ table, model, exists, columns, indexes }`. The same answer, per store, is what [the `schema` tool of `henri mcp`](/guides/agents/#asking-the-running-application) serves an agent.
...
| `i18n.catalogue`      | A file of `config/locales` is not JSON, or holds a value that is not text. It fails the boot.                                                                                                          | `HENRI_LOCALE_CATALOGUE_INVALID`                    |
...
| `deps.playwright`     | A `playwright.config.*` is here and `package.json` does not depend on `@playwright/test` (a warning): nothing loads the browser suite, and nothing says why.                                           | —                                                   |
...
**What the packages ship.** The documentation travels inside `@usehenri/core` (`scripts/prepublish.js` copies `website/src/content/docs` into it at publish time), which is what makes [`docs`](#docs) and the `guide` tool of [`mcp`](#mcp) answer for the henri this application runs rather than for whatever the website says today. Nothing checked that they arrived, and the reader does not fail when they did not: it falls through to the copy next to the command line, so an agent reads another version's documentation and corrects itself against it. `docs.version` (a warning) is that — the pages that would be printed come from another package than the `@usehenri/core` here, with both versions named — and `docs.missing` is the case where there is nothing anywhere, which is `HENRI_AGENT_NO_DOCS` the next time somebody asks for a page. Both name the directory and say what was wrong with it (not there, empty, unreadable). A `@usehenri/core` that is not installed at all says nothing here: `deps.declared` and `deps.installed` have already said it in the vocabulary a person can act on, and a checkout of henri itself reading `website/src/content/docs` is right rather than a finding.
...
Four of its checks read your source for a known-dangerous _shape_ by pattern; none of them follows a value from a request to a sink, so what it finds there is a shape and not an analysis ([Security](/guides/security/#what-henri-audit-does-not-do)). It reports what the application says, never henri's own defaults, which are secure: a protection turned off in `config/*.json`, a secret or a credentials key that reached a commit, a model write that takes the whole request body, a resource action left without a role where its siblings have one, a raw query built by interpolation, unescaped output in a view, a record answered as the ORM returned it, and the known advisories of the production dependencies.
...
Falling through is quiet by design — a page is better than no page — so [`doctor`](#doctor) is what says the fall-through happened: `docs.missing` when this application's `@usehenri/core` ships no pages and nothing else answers either, `docs.version` when something else does and the pages therefore describe another henri.
...
With `--json` a failure prints `{ "error": { "command", "message", "hint", "code", "exitCode" } }` on stderr. The `code` is one of [henri's error codes](/reference/errors/) — `HENRI_CLI_USAGE`, `HENRI_CONFIG_INVALID`, `HENRI_STORE_URL_MISSING` — which is finer grained than the exit code and is the same name the framework raises at runtime, so a boot failure keeps it all the way to the shell. `HENRI_CONFIG_INVALID` also carries a `problems` array (`{ key, level, message, expected, received, source, hint }`). The `hint` is what to do next: the failure's own when it has one, and otherwise the **how to fix it** of that code's [catalogue entry](/reference/errors/), so a coded failure never reaches the terminal as a name alone. See [Coding agents](/guides/agents/).
