...
- a `decimal` on an `@usehenri/mssql` store, whose tedious driver reads every `DECIMAL` through a JavaScript number
- a Sequelize `DECIMAL` with no precision, which MySQL makes `DECIMAL(10, 0)` -- whole units, so money loses its cents

**Fix.** The message names the model and the field. On sqlite, use `@usehenri/drizzle`, which stores both exactly, or put the store on postgres or mysql. On SQL Server a `bigint` is exact and a `decimal` is not, so store the amount in its smallest unit (cents) and format it in the application. For a bare `DECIMAL`, write `{ type: 'decimal', precision: 12, scale: 2 }` and henri writes the same column on every dialect.
...
The server-sent event streams of `res.stream()` and `henri.streams`: what they are named, what they carry and how many one process holds.

### `HENRI_STREAM_EVENT_INVALID`

The name or the id of a stream event cannot be written into a frame.

Usually:

- an `event` or an `id` holding a newline, a carriage return or another control character
- `henri.streams.publish(topic, event)` called with an empty event name

**Fix.** Name the event a plain word (`changed`, `proposal.updated`). henri refuses these rather than escaping them: a newline ends the field, so whoever chose the name would be writing raw event-stream fields into the frame.

### `HENRI_STREAM_POLICY_REQUIRED`

A stream was opened with no policy to ask, and henri opens none it cannot authorize.

Usually:

- `res.stream(topic)` on a route whose controller does not name a model
- no `subject` and no `policy`, so nothing said which `app/policies` file to ask

**Fix.** Pass the record the subscription is about (`res.stream(topic, { subject: proposal })`) or name the policy (`{ policy: "Proposal" }`). A stream that really is open to everybody says so on purpose, with `show: () => true` in `app/policies/<model>.js`.

### `HENRI_STREAM_TOO_MANY`

This process is already holding as many streams as it was told to.

Usually:

- more subscribers than `streams.maxOpen` (1000 by default) on this process
- streams that are never closed, because `streams.maxAge` was set to `false`

**Fix.** The subscriber is answered 503 with a `Retry-After` and comes back. Raise `streams.maxOpen` if the process can hold them, and check `streams.maxAge` is not `false` -- a stream that never ends is one that never gives its slot back.

### `HENRI_STREAM_TOPIC_INVALID`

A stream was given a topic henri will not address.

Usually:

- a topic that is not a string, is empty, is longer than 256 characters, or holds a control character

**Fix.** Build the topic in the controller out of something henri already publishes: `res.stream(`proposal:${proposal.externalId}`, { subject: proposal })`. A topic is never taken from a query string -- "subscribe to whatever you name" is the whole vulnerability.
