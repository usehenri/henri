...
| `assets`           | same origin   | `{ "prefix": "https://cdn.example.com" }`: where the files the production build wrote are loaded from. henri names the origin in the Content Security Policy itself. See [Views](/guides/views/#serving-the-assets-from-a-cdn).           |
...
| `csp`              | off           | Content Security Policy settings henri owns beside `helmet`: `add` and `nonce`, see below. See [Security](/guides/security/#content-security-policy).                                                                                     |
...
| `timeZone`         | `UTC`         | The zone a server renders a moment in, never what it stores; a name, or `{ default, from }`, see below. See [Time zones](/guides/time/).                                                                                                  |
...
`drizzle`, `postgresql`, `mysql` and `mariadb` are all `@usehenri/drizzle`: the last three are that adapter with the dialect and the driver chosen, so a store on one of them needs no `dialect` key and the application declares no driver. `mssql` is the one adapter on Sequelize, because Drizzle has no SQL Server dialect. A `mariadb` store needs `"sync": false` and cannot eager load; the two reasons are measured in [Models](/guides/models/#mariadb).
...
| `api.maxFilters`                | `8`                                                     | The most `filter[...]` terms one request may carry on a [declared index](/guides/filtering/).                                                                                                                             |
| `api.maxSort`                   | `3`                                                     | The most columns one request may order by (`?sort=a,-b`).                                                                                                                                                                 |
| `api.maxEmbeds`                 | `3`                                                     | The most relations one request may ask to [embed](/guides/api/#embedding-relations) (`?embed=lines,customer`).                                                                                                            |
| `api.maxEmbedded`               | `25`                                                    | The most records one `_embedded` relation carries per record, when the controller declares no `limit` of its own.                                                                                                         |
| `api.csv`                       | `{ "batch": 500, "formulas": true, "maxRows": 100000 }` | What [`res.csv()`](/guides/api/#exporting-a-csv) reads at once, whether a cell starting with `=`, `+`, `-` or `@` is written as text, and the most rows one export may carry.                                             |
...
| Key                | Default                                            | Description                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------ | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `helmet`           |                                                    | Options merged over henri's defaults (a development CSP that allows hot reloading, no HSTS outside production, `upgrade-insecure-requests` only on https requests), plus `permissionsPolicy` (a `Permissions-Policy` value, or `false`); `false` disables helmet. A policy directive written here **replaces** henri's array — `csp.add` is how you add one source and keep the rest. |
| `csp.add`          |                                                    | Sources to add to the policy henri built, by directive (`{ "script-src": ["https://plausible.io"] }`). The origin of `assets.prefix`, the nonce and the development sources stay. See [Content Security Policy](/guides/security/#content-security-policy).                                                                                                                           |
...
| Key                         | Default             | Description                                                                                                                                                                                                                                                     |
| --------------------------- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `uploads.storage`           | `"local"`           | `"local"` for the disk, `"s3"` for an object store ([`@usehenri/s3`](/guides/uploads/#an-object-store)), or the module id of a `HenriStorage` resolved from the application. An object (`{ "adapter": "s3", "bucket": … }`) names one and carries its settings. |
...
| `uploads.urls`              | `false`             | Signed urls: `{ "expiresIn": 300 }` turns them on, `false` hands out none and `henri.uploads.url()` refuses. See [the guide](/guides/uploads/#signed-urls).                                                                                                     |
...
Where the switch that closes the application lives, and what a visitor is told while it is thrown. Nothing here turns maintenance on -- `henri maintenance:on` does, from a shell, without a deploy. See [Maintenance mode](/guides/maintenance/).

| Key                      | Default                      | Description                                                                                                                                                                                                           |
| ------------------------ | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maintenance.switch`     | `auto`                       | Where the switch lives: `auto` is the [shared store](#the-shared-object) when `config.shared` names one and a file otherwise. `shared` and `file` pin one; the boot line says which is in play.                       |
| `maintenance.file`       | `.henri/maintenance.json`    | The file the switch is written to when it is not in the shared store. It reaches every process on that machine and no other.                                                                                          |
| `maintenance.poll`       | `1000`                       | Milliseconds a running process may go without re-reading the switch. Zero re-reads it on every request; the reads are deduplicated, so a burst costs one.                                                             |
| `maintenance.message`    | see below                    | What a visitor is told, unless `henri maintenance:on --message` says otherwise for one window.                                                                                                                        |
| `maintenance.retryAfter` | `300`                        | The `Retry-After` of the `503`, in seconds. `--retry-after` says it for one window.                                                                                                                                   |
| `maintenance.page`       | `app/views/maintenance.html` | An html page of your own, read as it is -- the view engine is not involved -- with `{{message}}`, `{{retryAfter}}` and `{{since}}` replaced by the escaped values. henri ships a page for when the file is not there. |
| `maintenance.readyz`     | `ready`                      | What `/readyz` answers while the application is closed. It stays ready on purpose; `unavailable` is for a deployment that wants every process pulled out of the load balancer at once.                                |
| `maintenance.bypass`     | `token`                      | What gets through besides the signed url `henri maintenance:on` prints. `loopback` also lets anything connecting from this machine through, which behind a proxy is everybody -- `henri audit` reports it.            |

`maintenance: false` gives the application no switch at all: no middleware, no read, and `henri maintenance:on` refuses with `HENRI_MAINTENANCE_DISABLED`.
