...
| `filterParameters` | `["password", "token", "secret", "authorization"]` | Substrings of the parameter names masked in everything `henri.pen` prints; `false` masks nothing. A name containing `encryption` is masked whatever this says, since the list replaces the defaults and the key that opens the encrypted columns must not depend on it. |
