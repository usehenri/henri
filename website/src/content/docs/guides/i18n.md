...
and on the wire, as `Content-Language`. When step 5 is what answered, the response also carries `Vary: Accept-Language`, so a cache in front of the application does not serve one visitor's language to another.
