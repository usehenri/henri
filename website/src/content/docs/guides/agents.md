...
| `schema`         | What the databases really hold: the tables, their columns and types, their indexes, and the tables no model claims.  |
...
### `schema` is the one `models` cannot answer

`models` reads the files, and a model file does not know what its table is called. The physical table name, the column a rename produced (`externalId` is `external_id` in every SQL store henri writes), the type the dialect chose for a `string`, whether a column is nullable, an index somebody added by hand, a table nothing declares any more -- all of that lives in the database and in the ORM objects of the running process. So an agent about to write SQL for the `query` tool should read `schema` first and never guess the names off `app/models`.

It is not `henri db:status` either, and the two are asked of the same database without being computed from one another: `db:status` says what is _wrong_ -- the migrations still pending, or, on a store with none, what the database and the models disagree about -- and says nothing at all when the answer is "nothing". `schema` says what is _there_. A SQL store carries both: the tables, plus a `drift` (the same report `db:status` prints, without the DDL -- nothing on this surface writes) or a `migrations` block, whichever its adapter keeps.

A `mongoose` store is the honest exception, and the answer says so rather than dressing a declaration up as a schema. Three of the four things it reports are real, server-side facts: which collections exist, which indexes each one carries, and which collections no model claims. The fourth, the fields, is not -- MongoDB validates nothing henri gave it -- so the answer carries `read: "models"`, `enforced: false` and a `note` saying that a document written by anything else can hold any shape at all.

Column _names_ are never masked: naming the columns is what the tool is for, and `password` is a column an agent has to be able to see. A column _default_ is a value, so it goes through the same rule everything else here does -- masked under a filtered name, and masked by shape if it looks like a hash or an encrypted column's envelope.

`henri db:schema` is the same answer for a person at a terminal, and it needs no running server. See [the CLI reference](/reference/cli/#db).
...
- **Redacted, by name and by shape.** What `filterParameters` masks in the logs is masked here, in the log lines, in the recorded parameters, in the query rows, in the records and in a schema's column defaults — never in a column's _name_, which is the one place a name has to survive; `password` is masked whatever the configuration says, and reading through the model keeps the adapter's own protections (a hash is not selected, a soft-deleted row does not come back). Those rules match a _key_, and the key of a query row is whatever the SQL said — `SELECT password AS p` renames the column to something no rule knows — so two values are read as well: a password hash and an encrypted column's envelope are masked under any name, at any depth. Neither can be a value you asked to see, so nothing legitimate is lost. It is not a general secret detector: a deliberate `SELECT` of something else still returns it, which is one of the reasons this surface exists in development only.
- **Bounded, and it says so.** 500 log lines kept, 25 errors, 100 rows a query, 25 records a page, 50 tables a store, 2000 characters a line, 40 stack frames. Anything cut carries `truncated: true` and the limit that cut it.
