...
| V4 Access control          | `roles` on a route registers a guard the router runs before the action: anonymous gets a redirect to `loginPath` or a `401`, signed in without the role a `403`. [Policies](/guides/policies/) answer the other question -- may this person act on _this_ record -- from `app/policies/<model>.js`, and they fail closed: no policy, no rule for the action and a rule that threw all mean no, and only the boolean `true` allows. A refusal answers `404` by default, so it says nothing about whether the record exists, and outside development its message is the reason phrase and nothing more -- a body reading "Not allowed to show this proposal" would give away in words what the status hides, so the reason reaches your logs and not the client. `res.notFound()` is the matching answer for a record that is not there, which is what `henri generate scaffold` writes: the pair is one answer, in body and in shape. An anonymous visitor gets a `401` and the login page instead, which is uniform only when the refusal was decided before the lookup: `"policies": { "anonymous": "uniform" }` answers them what a signed-in stranger gets, redirect included, at the cost of the login-page affordance -- a `roles` on the route decides at the gate and needs no key. The `paths` a view receives and the `_links` of every HAL answer are filtered by the roles of the viewer and then by the policies, so a page cannot link where its reader may not go. `roles` is stripped from every model write unless you pass `{ unsafe: true }`.                                                                                                                                         |
...
value), and the client secret of an
[identity provider](/guides/users/#signing-in-with-somebody-elses-identity-provider)
written in a `config/*.json` (`secret.identity-client`, high: whoever reads
the repository can complete a sign-in as this application; the finding names
the provider and never the secret).
...
**A provider believed about who owns an account** --
`user.identities.merge: "verified"` in a configuration a production boot
reads (`identities.merge-verified`, high). That setting links a callback
whose verified address already belongs to an account _to that account_, so
whoever can make the provider assert an address is handed the account behind
it, password and all. The default refuses and asks the person to sign in and
link the provider from their account, which is the only link that carries
their consent. The finding names the providers marked `trusted`. In the same
files, `user.identities.allowHttp: true` (`identities.http-allowed`, medium)
puts the client secret, the authorization code and the access token on a
plaintext connection; it is for a provider on the loopback in development and
nothing else.
