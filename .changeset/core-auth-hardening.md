...
- Double-submit CSRF protection: the `henri.csrf` cookie must be sent back as `X-CSRF-Token` or `_csrf` on unsafe requests carrying a session (`config.csrf: false` disables it, bearer tokens are exempt). The token is available as `req._henri.csrf` and `withHenri` adds the header to `fetch()` and `hydrate()`.
