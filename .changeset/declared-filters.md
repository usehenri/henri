...
**The scope wins.** `req.filter()` answers `policy.scope(user) AND (what the client asked for)` — an `and`, spelled for the adapter, so two conditions on one column intersect and a filter can only ever narrow a list. The scope is asked of the policy by default, so an action reaching for this on a model whose policy declares no scope gets the refusal `henri.policies.scope()` already gives; `req.filter({ scope: false })` is how an application says a list is public.
...
`Model.paginate()` on Mongoose now accepts `order` as the name of its `sort` option, so what `req.filter()` answers reads the same on every adapter.
