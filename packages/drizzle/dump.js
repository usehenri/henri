...
const postgres = async (query) => {
  ...
  // `attidentity` is 'a' for ALWAYS, 'd' for BY DEFAULT, '' for neither
  ...
  for (const [name, rows] of groupBy(columns, 'table_name')) {
    const body = rows.map((row) => {
      const identity = generated.get(row.identity) || '';
      const value =
...
/**
 * The tables a dump would create
 *
 * @param {Array<string>} statements The statements of a dump
 * @returns {Array<string>} The table names
 */
const tablesOf = (statements) =>
  statements
    .map((statement) =>
      /^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?["'`]?([^"'`\s(]+)/iu.exec(
        statement
      )
    )
    .filter(Boolean)
...
class Dump {
   ...
   *
   * It refuses a table it is about to create that already exists, and it
   * never empties a database to get its way: `henri db:drop` and
   * `henri db:create` are the commands that do that, and there is no flag
   * here, because a load that quietly took an application's tables with it
   * would be a worse version of them. A table the dump says nothing about
   * -- one another tool owns in the same database -- is left alone rather
   ...
   *   HENRI_MIGRATION_DATABASE_NOT_EMPTY when a table it would create is
   *   there already
   * @memberof Dump
   */
  async load() {
    ...
    if (!fs.existsSync(file)) {
      throw coded(
        ...
        'A dump is read from a database, so it is written where one is: a developer machine, or a CI job with a service container'
      );
    }
    ...
    if (at && !journal.entries.some((entry) => entry.tag === at)) {
      throw coded(
        ...
        'The dump is from another branch: take a new one, or check out the branch that has that migration'
      );
    }

    const db = adapter.rawDatabase();
    const statements = statementsOf(text);
    const present = new Set(await adapter.listTables());
    const clash = tablesOf(statements).filter((table) => present.has(table));

    if (clash.length > 0) {
      throw coded(
        'HENRI_MIGRATION_DATABASE_NOT_EMPTY',
        `drizzle: store ${adapter.name} already holds ${clash.length} of the table(s) the dump describes (${clash
          .slice(0, 5)
          .join(', ')})`,
        'A schema load creates them: empty the database first with "henri db:drop" then "henri db:create", or "henri db:reset" for the whole cycle'
      );
    }
...
module.exports = { Dump, HEADER, statementsOf, tablesOf };
