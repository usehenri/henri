...
/**
 * The client half of henri's i18n, for an Inertia page.
 *
 ...
 * engine and the Handlebars one. A page that wants markup writes it in JSX
 * around two keys.
 *
 * **How the strings get here.** A full document carries the catalogue in
 * the page props henri embedded; every Inertia visit after that carries
 * only `{ locale, source, url }`, because the browser already has the
 * strings and shipping them again on every visit is the design this one
 * exists to avoid. When the locale changes mid-session the props name a url
 * whose digest is in the file name, so the answer is cached forever and
 * fetched once.
 */
...
/**
 * Keeps the catalogue a document arrived with, so the visits after it do
 * not have to carry one
 *
 * @param {object} i18n the `i18n` prop of a page
...
export const useTranslation = () => {
  const page = usePage();
  const i18n = (page && page.props && page.props.i18n) || NO_LOCALE;
