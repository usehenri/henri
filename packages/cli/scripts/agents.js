...
- \`res.negotiate({ html, json })\` answers the page or the JSON; \`res.resource(post)\` and \`res.collection(posts, req.pagination())\` answer HAL with the links the roles and the policies allow. \`res.notFound(why)\` answers the 404 for a record that is not there -- use it rather than \`res.boom.notFound()\` for a lookup, because a policy refusal answers a 404 too and the reason is dropped in production so the two cannot be told apart. \`res.boom.badData\` (422), \`unauthorized\`, \`forbidden\` and \`conflict\` answer the error envelope.
